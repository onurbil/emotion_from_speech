...
def save_audio(DATA_PATH, name = 'recording.wav'):
    data_list = []
    cls = 'ANY'
    ## Say the word calm
    ...
    scipy.io.wavfile.write(name, sr, audio)  # Saving audio
    sound = am.from_file(name, format='wav', frame_rate=48000, start_second=0.2, duration=1.9)
    sound = sound.set_frame_rate(24414)
    sound.export(name, format='wav')
    data, sr = librosa.load(name, sr=24414)
